...
def test(model, test_dataloader):
    ...
    with torch.no_grad():
        for t_sample, label in tqdm.tqdm(test_dataloader):
            ...
            t_correct_1 = torch.eq(torch.argmax(class_output_1, 1),torch.argmax(label, 1))  # get the index of the max log-probability
...
def train(model, ema_model,optimizer, train_dataloader, test_dataloader, test_person_id):
    ...
    for epoch in range(args.epoch):
        ...
        for x, label in tqdm.tqdm(train_dataloader):
            ...
            t_correct_1 = torch.eq(torch.argmax(label_pred_1, 1),torch.argmax(label, 1))  # get the index of the max log-probability
        ...
        test_info = 'Test acc_1 Epoch_{}: {:.4f}, acc_2 :{:.4f}, acc_3 :{:.4f}, acc_m :{:.4f}'.format(epoch,
                                                                                                      test_acc_1,
                                                                                                      test_acc_2,
                                                                                                      test_acc_3,
                                                                                                      test_acc_m)
...
def main(args, device):
    data_path = "F:\\wsy\\研一\\科研\\Mymodel\\MT-MTDA - subject - one - WFE\\datasets\\SEED_channel_32_time_3\\session_1_32\\"
    l_index = [i for i in range(15)]
    sumT = 0

    for j in range(14):
        print_info = "=================模型{}=======================\n".format(j)
        print(print_info)
        model_path = r"F:/wsy/研一/科研/Mymodel/MT-MTDA - subject - one - WFE/models_one/one_model_%d.pth" % j
        ...
        num_classes = 3  # 新任务的类别数量
        model.fcs = nn.ModuleList([nn.Linear(model.fcs[0].in_features, num_classes) for a in range(3)])

        for index, i in enumerate(l_index):

            # 样本分类 ： train & test
            for j in range(15):
                content_label = np.load(data_path + "label.npy")
                content_data = np.load(data_path + "person_%d DE.npy" % (j)).transpose(1, 0, 2, 3)
                # test
                if j == i:
            ...
            test_dataloader = DataLoader(test_data, batch_size=args.batch_size, num_workers=4)
            ...
            for trail_num in range(1):
                ...
                optimizer = torch.optim.SGD(model.fcs.parameters(), lr=args.lr, momentum=0.9)  # TODO: Modified
                ...
                # file_path = os.path.join(save_dir, f"one_model_{i}.pth")
                # torch.save( { 'model_state_dict': model.state_dict(),
                #               'optimizer_state_dict': optimizer.state_dict(),
                #              }, file_path)  # save: state_dict() load: load_state_dict()
                print(test_info)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ema-weight', type=float, default=1.5)
