...
def get_config_var():
    ...
    path = "F:\\wsy\\研一\\科研\\Mymodel\\MT-MTDA\\save_dir"
    if not os.path.exists(path):
        vars["SAVE_DIR"] = os.makedirs("F:\\wsy\\研一\\科研\\Mymodel\\MT-MTDA\\save_dir")
...
def eval(model, device, test_loader, is_debug=False):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            #data = data.reshape(-1,32,20)
            data, target = data.to(device), target.to(device)
            output_1, output_2, output_3, output_m = model.nforward(data)
            t_correct_1 = torch.eq(torch.argmax(output_1, 1), torch.argmax(target, 1)) # get the index of the max log-probability
            t_correct_2 = torch.eq(torch.argmax(output_2, 1), torch.argmax(target, 1))
            t_correct_3 = torch.eq(torch.argmax(output_3, 1), torch.argmax(target, 1))
            t_correct_m = torch.eq(torch.argmax(output_m, 1), torch.argmax(target, 1))
    ...
    acc_1 = 100. * correct_1 / len(test_loader.dataset)
    acc_2 = 100. * correct_2 / len(test_loader.dataset)
    acc_3 = 100. * correct_3 / len(test_loader.dataset)
    acc_m = 100. * correct_m / len(test_loader.dataset)

    del output_1, output_2, output_3, output_m
    return acc_1, acc_2, acc_3, acc_m
