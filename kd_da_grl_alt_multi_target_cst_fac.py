...
def grl_multi_target_hinton_train_alt(current_ep, epochs, teacher_models, student_model, optimizer_das, optimizer_kd, device,
    ...
    for i in range(1, len(source_dataloader) + 1):
        ...
        for ix, it in enumerate(iter_targets):
            ...
            teacher_source_loss_cls = F.cross_entropy(F.softmax(teacher_label_source_pred, dim=1), label_source)

            #data_target_tem = data_target.reshape(64,32,-1)
            ...
            teacher_da_grl_loss = (1 - beta) * (teacher_source_loss_cls + gamma * teacher_loss_adv) #1-β LDA
            ...
            teacher_target_logits, _ = teacher_models[ix](data_target, delta, source=False)
            ...
            kd_loss = beta * (target_kd_loss + source_kd_loss) #total loss第二项
