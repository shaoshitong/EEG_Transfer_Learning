...
from torch.utils.model_zoo import load_url as load_state_dict_from_url#给定URL加载Torch序列化对象
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):#如果对象的类型与参数二(classinfo)的类型相同返回true，否则false。
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')#初始化方法，kaiming均匀分布
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):#_make_layer()函数用来产生layer的个数 然后layer里的block数目不同
